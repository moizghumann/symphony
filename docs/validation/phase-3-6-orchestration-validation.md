...
Layer A unit tests and Layer B local dry-run simulations pass. The required 2026-05-11 non-mutating GitHub/Linear preflight now passes, and local validation was rerun successfully. Layer C real Linear/GitHub smoke tests are still not run because this repo does not yet contain an existing safe Phase 3.6 live-smoke command or test harness that runs only the first three lanes and collects the required evidence without improvising unsafe mutations.

## Test Matrix

| Lane | Unit tests | Dry run | Real smoke | Result |
|---|---|---|---|---|
| docs | pass | pass | not run, no safe runner | partial |
| bug | pass | pass | not run, no safe runner | partial |
| feature | pass | pass | not run | partial |
| refactor | pass | pass | not run | partial |
| test | pass | pass | not run, no safe runner | partial |
| chore | pass | pass | not run | partial |
| research | pass | pass | not run, no safe runner | partial |
...
The auth blocker from the prior attempt is resolved. Live smoke did not proceed because no existing safe Phase 3.6 runner or command was found for exactly the first three lanes: docs, bug or test, and research.
...
Smallest safe opt-in live-smoke runner to add before mutation:

- Refuse to run unless `RUN_REAL_SMOKE=true`.
- Refuse to run unless `LINEAR_API_KEY` is present.
- Refuse to run unless `GH_TOKEN` or `GITHUB_TOKEN` is present.
- Run and pass non-mutating GitHub preflight before any mutation:
  - `gh auth status`
  - `gh repo view moizghumann/symphony --json nameWithOwner`
- Run and pass non-mutating Linear preflight before any mutation:
  - viewer query
  - `Agent Workbench` team/state query
  - exact status verification with `Canceled`, not `Cancelled`
- Create or select only three live-smoke tickets for docs, bug or test, and research.
- Stop after those three lanes; leave feature, refactor, chore, and the unselected bug/test counterpart not run.
- For each smoke run, persist Linear identifier, lane, classification reason, final state, PR URL if repo changed, changed files, validation status, effective tokens, gross context tokens, cached input tokens, tool-call count, generic Linear GraphQL calls, narrow Linear lifecycle calls, budget state, finalization gate result, protocol violations, and protocol warnings.
- Enforce docs behavior: repo-changing docs-only ticket, draft PR required, Human Review only after PR URL posted, generic Linear GraphQL = 0 on happy path, effective tokens <= 30k target.
- Enforce bug/test behavior: failure signal or targeted test behavior required, validation behavior must match lane, draft PR required if repo changed.
- Enforce research behavior: read-only by default, no repo file changes, no PR unless artifact requested, findings posted, Human Review only after findings handoff.
...
- Add the smallest safe opt-in Phase 3.6 live-smoke runner described above, or identify an existing equivalent command if one is added elsewhere.
...
Do not mark Phase 3.6 fully passed until real smoke tests run. Phase 3.6 remains partial: preflight and local validation are green, but Layer C still needs a safe runner and real smoke evidence for docs, bug or test, and research before expanding to the remaining lanes.
