defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server forwards plain text turn output to the orchestrator callbacks" do
    test_root =
      Path.join(
        System.tmp_dir!(),
    ...
    try do
      ...
      issue = %Issue{
        ...
        identifier: "MT-94",
        title: "Surface stream output",
        description: "Ensure plain text stream output reaches callbacks",
        state: "In Progress",
      ...
      assert {:ok, _result} =
               AppServer.run(workspace, "Surface plain text output", issue, on_message: on_message)

      assert_received {:app_server_message, %{event: :stream_output, raw: "Repository work complete."}}
      assert_received {:app_server_message, %{event: :stream_output, raw: "SYMPHONY_HANDOFF_READY"}}
      assert_received {:app_server_message, %{event: :turn_completed}}
    after
      File.rm_rf(test_root)
    end
